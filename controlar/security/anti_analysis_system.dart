...
class AntiAnalysisSystem {
  ...
  bool? _isRootedCached;
  bool? _isJailbrokenCached;
  DateTime? _lastRootCheck;
  ...
  Future<void> _loadState() async {
    try {
      ...
      final detectionCounter = await _secureStorage.read(key: KEY_DETECTION_COUNTER);
      ...
      final detectionEvents = await _secureStorage.read(key: KEY_DETECTION_EVENTS);
  ...
  Future<void> _saveState() async {
    try {
      // Save stealth mode
      await _secureStorage.write(key: KEY_STEALTH_MODE, value: _inStealthMode.toString());
      
      // Save detection counter
      await _secureStorage.write(key: KEY_DETECTION_COUNTER, value: _detectionCounter.toString());
      
      // Save last detection time
      if (_lastDetection != null) {
        await _secureStorage.write(key: KEY_LAST_DETECTION, value: _lastDetection!.toIso8601String());
      }
      ...
      if (_detectionEvents.isNotEmpty) {
        final events = _detectionEvents.length > 10 ? 
            ...
            _detectionEvents;
        await _secureStorage.write(key: KEY_DETECTION_EVENTS, value: events.join(','));
      }
    } catch (e) {
  ...
  Future<void> _runPeriodicCheck() async {
    try {
      ...
      if (analysisDetected) {
        debugPrint('AntiAnalysisSystem: Analysis attempt detected in periodic check');
        ...
        if (_detectionCounter >= 3) {
          await enterStealthMode(triggeredBy: 'periodic_check_multiple_detections');
        }
      }
  ...
  Future<void> _enforceStealthMode() async {
    ...
    debugPrint('AntiAnalysisSystem: Stealth mode enforced for ${stealthDuration.inHours} hours');
  }
  ...
  Future<bool> detectAnalysisAttempt() async {
    ...
    try {
      ...
      if (detected) {
        _lastDetection = DateTime.now();
        _detectionEvents.add('${DateTime.now().toIso8601String()}: ${detections.join("; ")}');
        await _saveState();
      }
  ...
  Future<void> enterStealthMode({String? triggeredBy}) async {
    ...
    triggeredBy ??= 'manual';
    debugPrint('AntiAnalysisSystem: Entering stealth mode (triggered by: $triggeredBy)');
    
    try {
      _inStealthMode = true;
      _detectionCounter++;
      _lastDetection = DateTime.now();
      _detectionEvents.add('${DateTime.now().toIso8601String()}: entered_stealth_mode: $triggeredBy');
  ...
  Future<bool> _isRootedOrJailbroken() async {
    // Use cached result if available and fresh (less than 3 hours old)
    if ((_isRootedCached != null || _isJailbrokenCached != null) && _lastRootCheck != null) {
      final difference = DateTime.now().difference(_lastRootCheck!);
      if (difference.inHours < 3) {
        if (Platform.isAndroid) {
          return _isRootedCached ?? false;
        } else if (Platform.isIOS) {
          return _isJailbrokenCached ?? false;
        }
      }
    }
    ...
    try {
      ...
      if (Platform.isAndroid) {
        // Use RootChecker library
        isCompromised = await RootChecker.isDeviceRooted ?? false;
        
        // Additional checks
        if (!isCompromised) {
          // Check for common root-only files
          final busyboxExists = await _fileSystemService.fileExists('/system/bin/busybox') ||
                                await _fileSystemService.fileExists('/system/xbin/busybox');
          final suExists = await _fileSystemService.fileExists('/system/bin/su') ||
                          await _fileSystemService.fileExists('/system/xbin/su') ||
                          await _fileSystemService.fileExists('/sbin/su');
          
          isCompromised = busyboxExists || suExists;
        }
        
        // Cache result
        _isRootedCached = isCompromised;
      } else if (Platform.isIOS) {
        // Use JailbreakDetection library
        isCompromised = await FlutterJailbreakDetection.jailbroken;
        
        // Cache result
        _isJailbrokenCached = isCompromised;
      }
      
      _lastRootCheck = DateTime.now();
      return isCompromised;
    } catch (e) {
      debugPrint('AntiAnalysisSystem: Error checking for root/jailbreak: $e');
      return false;
    }
  }
      ...
      if (Platform.isAndroid) {
        ...
        for (var app in apps) {
          ...
          for (var toolPattern in ANALYSIS_TOOL_PATTERNS) {
            if (packageName.contains(toolPattern) || appName.contains(toolPattern)) {
        ...
        if (await _fileSystemService.fileExists('/data/local/tmp/frida-server')) {
          detectedTools.add('frida-server');
        }
        
        if (await _fileSystemService.fileExists('/system/lib/libxposed_art.so') ||
            await _fileSystemService.fileExists('/system/lib64/libxposed_art.so')) {
  ...
  Future<bool> _checkSuspiciousEnvironment() async {
    try {
      if (Platform.isAndroid) {
        ...
        try {
          final envVars = await methodChannel.invokeMethod<Map>('getEnvironmentVariables');
  ...
  Future<bool> _detectNetworkMonitoring() async {
    try {
      ...
      try {
        final proxySettings = await methodChannel.invokeMethod<Map>('getProxySettings');
      ...
      try {
        ...
        final sslCheck = await securityChannel.invokeMethod<bool>('checkForSSLPinningBypass');
      ...
      debugPrint('AntiAnalysisSystem: Error checking for network monitoring: $e');
    }
